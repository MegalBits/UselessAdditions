...
public class TraderSatchel extends Item {
    ...
    public static float getAmountFilled(ItemStack stack) {
        return (float) TraderSatchel.getBundleOccupancy(stack) / MAX_STORAGE;
    }
    ...
    public boolean onClicked(ItemStack stack, ItemStack otherStack, Slot slot, ClickType clickType, PlayerEntity player, StackReference cursorStackReference) {
        ...
        if (otherStack.isEmpty()) {
            TraderSatchel.removeFirstStack(stack).ifPresent(itemStack -> {
                this.playRemoveOneSound(player);
                cursorStackReference.set((ItemStack)itemStack);
            });
        } else {
            int i = TraderSatchel.addToBundle(stack, otherStack);
            if (i > 0) {
                this.playInsertSound(player);
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (TraderSatchel.dropAllBundledItems(itemStack, user)) {
    ...
    public boolean isItemBarVisible(ItemStack stack) {
        return TraderSatchel.getBundleOccupancy(stack) > 0;
    }

    @Override
    public int getItemBarStep(ItemStack stack) {
        return Math.min(1 + 12 * TraderSatchel.getBundleOccupancy(stack) / MAX_STORAGE, 13);
    }
    ...
    private static int addToBundle(ItemStack bundle, ItemStack stack) {
        ...
        int i = TraderSatchel.getBundleOccupancy(bundle);
        int j = TraderSatchel.getItemOccupancy(stack);
        int k = Math.min(stack.getCount(), (MAX_STORAGE - i) / j);
        if (k == 0) {
            return 0;
        }
        NbtList nbtList = nbtCompound.getList(ITEMS_KEY, NbtElement.COMPOUND_TYPE);
        Optional<NbtCompound> optional = TraderSatchel.canMergeStack(stack, nbtList);
    ...
    private static Optional<NbtCompound> canMergeStack(ItemStack stack, NbtList items) {
        if (stack.getItem() instanceof BundleItem || stack.getItem() instanceof TraderSatchel) {
    ...
    private static int getBundleOccupancy(ItemStack stack) {
        return TraderSatchel.getBundledStacks(stack).mapToInt(itemStack -> TraderSatchel.getItemOccupancy(itemStack) * itemStack.getCount()).sum();
    }
    ...
    public Optional<TooltipData> getTooltipData(ItemStack stack) {
        ...
        TraderSatchel.getBundledStacks(stack).forEach(defaultedList::add);
        return Optional.of(new BundleTooltipData(defaultedList, TraderSatchel.getBundleOccupancy(stack)));
    }
    ...
    public void onItemEntityDestroyed(ItemEntity entity) {
        ItemUsage.spawnItemContents(entity, TraderSatchel.getBundledStacks(entity.getStack()));
    }
