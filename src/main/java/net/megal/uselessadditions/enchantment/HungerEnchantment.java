...
public class HungerEnchantment extends AugmentEnchantment {
    ...
    public void onTargetDamaged(LivingEntity user, Entity target, int level) {
        int amplifier = 0;
        if (level >= 4) amplifier++;
        int duration = (level-1) * 40 + 100;
        if (target instanceof LivingEntity) ((LivingEntity) target).addStatusEffect(new StatusEffectInstance(StatusEffects.HUNGER, duration, amplifier));
    }
    @Override
